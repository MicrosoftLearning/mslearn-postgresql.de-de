...
In diesem Abschnitt erkunden Sie den Vakuumprozess in PostgreSQL. Der Vakuumprozess wird verwendet, um Speicherplatz zurückzugewinnen und die Leistung der Datenbank zu optimieren. Sie können den Vakuumprozess manuell ausführen oder so konfigurieren, dass er automatisch ausgeführt wird.

1. Wählen Sie im Visual Studio Code-Fenster **Datei**, **Datei öffnen** und navigieren Sie dann zu den Lab-Skripten. Wählen Sie **../Allfiles/Labs/07/Lab7_vacuum.sql** und wählen Sie dann **Öffnen**. Stellen Sie gegebenenfalls die Verbindung zum Server wieder her, indem Sie den Text **PGSQL Disconnected** auswählen und anschließend Ihre PostgreSQL-Serververbindung aus der Liste in der Befehlspalette auswählen. Wenn Sie nach einem Kennwort gefragt werden, geben Sie das zuvor erstellte Kennwort ein.
...
In diesem Abschnitt konfigurieren Sie die Parameter für den Autovacuum-Server. Der Autovacuum-Prozess wird verwendet, um automatisch Speicherplatz zurückzugewinnen und die Leistung der Datenbank zu optimieren. Sie können den Autovacuum-Prozess so konfigurieren, dass er automatisch basierend auf bestimmten Parametern ausgeführt wird.

1. Navigieren Sie zum [Azure-Portal](https://portal.azure.com), falls Sie dies noch nicht getan haben.
...
1. Geben Sie in der Suchleiste **`vacuum`** ein. Suchen Sie die folgenden Parameter, und ändern Sie die Werte wie folgt:
    ...
    - autovacuum = ON (sollte standardmäßig „ON“ lauten)
    - autovacuum_vacuum_scale_factor = 0.1
    - autovacuum_vacuum_threshold = 50

    Diese Änderungen entsprechen der Ausführung des Autovacuum-Prozesses, wenn 10 Prozent einer Tabelle zum Löschen von Zeilen markiert sind oder 50 Zeilen in einer beliebigen Tabelle aktualisiert oder gelöscht wurden.

1. Wählen Sie **Speichern**. Der Server wird neu gestartet.

## Aufgabe 3: Anzeigen von PostgreSQL-Metadaten im Azure-Portal

In diesem Abschnitt sehen Sie die PostgreSQL-Metadaten im Azure-Portal. Das Azure-Portal bietet eine grafische Schnittstelle zum Verwalten und Überwachen Ihres PostgreSQL-Servers.
...
In diesem Abschnitt zeigen Sie Daten in Systemkatalogtabellen an. Systemkatalogtabellen werden verwendet, um Metadaten zu den Datenbankobjekten in PostgreSQL zu speichern. Sie können diese Tabellen abfragen, um Informationen zu den Datenbankobjekten abzurufen.
...
In diesem Abschnitt zeigen Sie eine komplexe Metadatenabfrage mithilfe einer Systemansicht an. Systemansichten werden verwendet, um eine vereinfachte Schnittstelle zum Abfragen von Metadaten zu den Datenbankobjekten in PostgreSQL bereitzustellen. 
...
1. Mithilfe von Systemsichten können Sie die Komplexität des SQL-Codes reduzieren, den Sie schreiben müssen. Die vorherige Abfrage benötigt den folgenden Code, wenn Sie die Sicht **pg_stats** nicht verwenden. Führen Sie also den folgenden Code aus, um zu sehen, wie es funktioniert. Kopieren Sie im Fenster **Neue Abfrage **die folgende SQL-Anweisung, markieren Sie sie und führen Sie sie aus:
...
In dieser Übung haben Sie erfahren, wie Sie Systemparameter konfigurieren und Metadaten in PostgreSQL erkunden. Außerdem haben Sie erfahren, wie Sie PostgreSQL-Metadaten im Azure-Portal anzeigen und Daten in Systemkatalogtabellen anzeigen. Darüber hinaus haben Sie erfahren, wie Sie eine komplexe Metadatenabfrage mithilfe einer Systemansicht anzeigen.
